...
func UpdateNoteFromDb(noteID int, notesRoot string, db *sql.DB) error {
  ...
  note := parser.ParseNorg(content, path)
...
func UpdateNotesFromDb(opts UpdateOptions) {
	if opts.All {
		...
		for rows.Next() {
			...
			if err := rows.Scan(&id); err == nil {
				UpdateNoteFromDb(id, opts.NotesRoot, opts.DB)
			}
		}
	...
	if opts.ID != nil {
		UpdateNoteFromDb(*opts.ID, opts.NotesRoot, opts.DB)
		return
	}
